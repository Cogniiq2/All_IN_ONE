...
/**
 * One already-imported reservation, chosen for a diagnostic read.
 *
 * ── Why this reads the local table rather than searching the provider ────
 * The financial probe must run against a reservation BoLaGio has already
 * imported and can point at — not against whatever a provider search happens
 * to return first. Reading `bolagio_reservations` means the caller names a
 * channel, the database names the booking, and the provider is asked for
 * exactly that one id.
 *
 * It returns identifiers and states only. No guest column is selected, so
 * there is no path from here to a name or an address even by accident.
 */
export interface ProbeReservationRef {
  externalBookingId: string;
  source: ReservationSource;
  statusClass: ReservationClass;
  providerStatus: string;
...
export async function findReservationForProbe(input: {
  source: ReservationSource;
  /** Pin one reservation instead of taking the most recent of the channel. */
  externalBookingId?: string;
}): Promise<ProbeReservationRef | null> {
  let query = supabaseAdmin()
    .from('bolagio_reservations')
    .select('external_booking_id, source, status_class, provider_status')
    .eq('provider', 'beds24')
    .eq('source', input.source);
  if (input.externalBookingId) query = query.eq('external_booking_id', input.externalBookingId);

  const { data, error } = await query.order('check_in', { ascending: false }).limit(1);
  if (error) throw error;
  const row = ((data ?? []) as unknown as Array<Record<string, string>>)[0];
  if (!row) return null;
  return {
    externalBookingId: row.external_booking_id,
    source: row.source as ReservationSource,
